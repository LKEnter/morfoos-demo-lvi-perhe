...
function ContactIcon({ children }: { children: React.ReactNode }) {
  return (
    <span
      className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[var(--color-accent-tint)] text-[var(--color-accent)]"
      aria-hidden="true"
    >
        ...
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:gap-16">
          <div>
            ...
            <dl className="mt-9 flex flex-col gap-5">
              <div className="flex items-center gap-3">
                <dt className="flex items-center gap-3">
                  ...
                  <span className="sr-only">Puhelin</span>
                </dt>
                <dd>
                  <a
              ...
              <div className="flex items-center gap-3">
                <dt className="flex items-center gap-3">
                  ...
                  <span className="sr-only">Toimipiste ja myymälä</span>
                </dt>
              ...
              <div>
                ...
                <dd className="mt-2 flex flex-wrap gap-2">
                  {SERVICE_AREA.map((town) => (
                    <span
                      key={town}
                      className="rounded-full border border-[var(--color-border)] bg-[var(--color-bg-light)] px-3.5 py-1.5 text-[14px] font-medium text-[var(--color-ink)]"
                    >
                      {town}
                    </span>
                  ))}
            ...
            <div className="reveal mt-9 min-h-[280px] overflow-hidden rounded-2xl border border-[var(--color-border)]">
              <iframe
                src={MAP_EMBED_SRC}
                title="Perheputki Etelä Oy kartalla"
